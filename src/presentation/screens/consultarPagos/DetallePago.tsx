...
export const DetallePago = ({ route, navigation }: any) => {
    ...
    const getPagosByCobrador = () => {

        const today = getCurrentDateDDMMYYYY();
        const pagosCobrador = pagos.filter((item: any) => item.usuario._id == idCobrador);
        const pagosToday = pagosCobrador.filter((item: any) => formatDateDDMMMYYY(item.fecha) == today);
    ...
    return (
        <View style={{ flex: 1, backgroundColor: '#FFF' }}>
            <Header title={`Pagos realizados por ${cobrador}`} color={'#adbc5b'} textColor={'#FAFAFA'}/>
            {
                loading ? <>
                    {
                        ...
                        <ScrollView>
                            ...
                            <View style={{ marginTop: 5 }}>
                                <View style={{ backgroundColor: '#d84473', height: 35, justifyContent: 'center' }}>
                                    <Text style={{ color: '#FFF', fontWeight: '900', textAlign: 'center', fontSize: 15 }}>
                                        {`Total cobrado`}
                                    </Text>
                                </View>
                            </View>
                        ...
                        :<View style={{ marginTop: 150 }}>
                            <Text style={{ textAlign: 'center', fontWeight: 'bold', fontSize: 16 }}>No se encontraron pagos.</Text>
                        </View>
